...
def fetch_jira_attachment(issue_id: str, target_file: str) -> str:
    ...
    auth = HTTPBasicAuth(
        username=os.getenv("JIRA_EMAIL"), password=os.getenv("JIRA_API_TOKEN")
    )
    headers = {"Accept": "application/json"}

    res = requests.get(
        url=f"{jira_url}/rest/api/3/issue/{issue_id}", auth=auth, headers=headers
    )
    ...
    if attachment_url:
        jira_attachment_data = requests.get(
            url=attachment_url, auth=auth, headers=headers
        )
...
def get_llm(provider: Optional[str] = None) -> LLM:
    ...
    if provider == "mistral":
        ...
        return LLM(model="ollama/qwen2.5-coder:32b", base_url="http://localhost:11434")
    else:
