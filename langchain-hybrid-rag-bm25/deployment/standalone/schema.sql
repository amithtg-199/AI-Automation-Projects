...
-- ────────────────────────────────────────────────────────────────────────────
-- 3. Evaluation Feedback & Loop Engineering Store
-- ────────────────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS evaluation_feedback (
    ...
    human_status      VARCHAR(50) DEFAULT 'PENDING',
    reviewer_notes    TEXT,
    created_at        TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at        TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

CREATE TABLE IF NOT EXISTS prompt_versions_meta (
...
CREATE INDEX IF NOT EXISTS idx_parent_chunks_search_vector ON parent_chunks USING GIN(search_vector);
CREATE INDEX IF NOT EXISTS idx_document_chunks_parent     ON document_chunks(parent_id);
CREATE INDEX IF NOT EXISTS idx_eval_feedback_project      ON evaluation_feedback(project_name);
