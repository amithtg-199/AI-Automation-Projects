...
def get_llm(for_eval: bool = False):
    """
    Returns the appropriate LangChain ChatModel based on config.LLM_PROVIDER.
    """
    provider = config.LLM_PROVIDER.lower()
    model_name = config.EVAL_MODEL_NAME if for_eval else config.LLM_MODEL_NAME
...
def get_embeddings():
    ...
    Cached to prevent reloading models in memory.
    """
...
def get_reranker(model_name: str = "BAAI/bge-reranker-base"):
    """
    Returns a Singleton instance of the BGE CrossEncoder re-ranker.
    """
    global _RERANKER_INSTANCE
    if _RERANKER_INSTANCE is None:
        logger.info(f"Initializing CrossEncoder Re-ranker ({model_name})...")
        try:
            from sentence_transformers import CrossEncoder
            _RERANKER_INSTANCE = CrossEncoder(model_name)
        except Exception as e:
            logger.warning(f"Failed to initialize sentence-transformers CrossEncoder ({e}). Re-ranker fallback to simple scoring.")
