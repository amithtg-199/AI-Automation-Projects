...
class AdaptiveRateLimiter:
    def __init__(self):
        ...
        self._max_retries = max(5, config.LLM_MAX_RETRIES)
    ...
    def invoke(self, chain, input_vars: dict, *, label: str = ""):
        ...
        for attempt in range(1, self._max_retries + 1):
            ...
            if current_delay > 0:
                logger.debug(f"Rate-limiter: waiting {current_delay:.1f}s before request ({label})")
            ...
            try:
                ...
                if self._is_retryable_error(exc):
                    with self._lock:
                        self._delay = min(max(4.0, self._delay * 2 or 4.0), self._max_delay)
                        wait_time = self._delay
                    logger.warning(
                        f"429 / Retryable error ({label}). "
                        f"Waiting {wait_time:.1f}s before retry {attempt}/{self._max_retries}. "
                        f"Error: {exc}"
                    )
                else:
