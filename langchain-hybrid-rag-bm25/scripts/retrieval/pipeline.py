...
from scripts.config import config, PROJECT_ROOT
...
class RetrievalPipeline:
    ...
    def retrieve_context(self, query: str, top_k: int = 5):
        """
        Retrieves high-precision context using Hybrid RAG:
        1. Dense Vector Search (Qdrant)
        2. Sparse Keyword Search (PostgreSQL BM25)
        3. Reciprocal Rank Fusion (RRF)
        4. CrossEncoder Re-ranking (BGE)
        """
        project_id = self.db.get_or_create_project(self.project_name)
        latest_ver = self.db.get_latest_version(project_id)
        latest_vid = latest_ver.get("version_id", "")
        ...
        if latest_vid:
            ...
            qdrant_filter = qmodels.Filter(
                must=[qmodels.FieldCondition(key="version_id", match=qmodels.MatchValue(value=latest_vid))]
            )
        try:
            dense_points = self.qdrant.query_points(
            ...
            logger.warning(f"Qdrant query failed ({e}), falling back to BM25 only.")
            dense_points = []

        logger.info(f"[Dense Vector Search] Retrieved {len(dense_points)} child chunks from Qdrant dense vector space.")

        # Map dense rank scores
        dense_ranks = {}
        for idx, hit in enumerate(dense_points):
            pid = hit.payload["parent_id"]
            dense_ranks[pid] = idx + 1

        # 2. Sparse BM25 Search (Top 20)
        bm25_hits = self.db.search_parent_chunks_bm25(project_id, query, limit=20)
        logger.info(f"[BM-25 Retrieval] Retrieved {len(bm25_hits)} parent chunks from PostgreSQL tsvector exact keyword index.")
        bm25_ranks = {}
        for idx, hit in enumerate(bm25_hits):
            pid = str(hit["parent_id"])
            bm25_ranks[pid] = idx + 1

        # 3. Reciprocal Rank Fusion (RRF, k=60)
        all_pids = set(dense_ranks.keys()).union(set(bm25_ranks.keys()))
        rrf_scores = {}
        for pid in all_pids:
            score = 0.0
            if pid in dense_ranks:
                score += 1.0 / (60.0 + dense_ranks[pid])
            if pid in bm25_ranks:
                score += 1.0 / (60.0 + bm25_ranks[pid])
            rrf_scores[pid] = score

        top_rrf = max(rrf_scores.values()) if rrf_scores else 0.0
        logger.info(f"[Reciprocal Rank Fusion (RRF)] Combined {len(all_pids)} unique candidates using RRF (k=60). Max score: {top_rrf:.4f}.")

        # Top 20 candidates from RRF
        sorted_pids = sorted(rrf_scores.keys(), key=lambda x: rrf_scores[x], reverse=True)[:20]
        ...
        # 4. CrossEncoder Re-ranking (BGE) Top 5
        from scripts.llm_factory import get_reranker
        reranker = get_reranker()
        if reranker != "FALLBACK" and hasattr(reranker, "predict"):
            pairs = [[query, block["content"]] for block in candidate_blocks]
            try:
                scores = reranker.predict(pairs)
                for idx, block in enumerate(candidate_blocks):
                    block["rerank_score"] = float(scores[idx])
                candidate_blocks.sort(key=lambda x: x.get("rerank_score", 0.0), reverse=True)
            except Exception as e:
                logger.warning(f"Re-ranker scoring failed ({e}), keeping RRF order.")
        
        top_blocks = candidate_blocks[:top_k]

        context_blocks = []
        citations = []
        for block in top_blocks:
            context_blocks.append(f"Source: {block['file_name']} ({block['section_name']})\n{block['content']}")
            citations.append({
    ...
    def repair_csv_content(self, text: str, filename: str) -> str:
        ...
        for line in text.splitlines():
            ...
            elif clean_lines and (re.match(r'^\d+[\.\)]\s+', line_str) or line_str.startswith(('-', '*')) or line_str.count(',') < 3):
                is_continuation = True

            if is_continuation and clean_lines:
                clean_lines[-1] += "; " + line_str
                quote_count += line_str.count('"')
            else:
                clean_lines.append(line_str)
                quote_count = line_str.count('"')

        for i in range(len(clean_lines)):
            if clean_lines[i].count('"') % 2 != 0:
                clean_lines[i] += '"'

        text_cleaned = "\n".join(clean_lines)
        reader = csv.reader(io.StringIO(text_cleaned))
        try:
            rows = list(reader)
        except Exception as e:
            logger.warning(f"Error parsing CSV reader in repair: {e}")
            return text
        ...
        for idx, row in enumerate(rows[1:], start=2):
            ...
            if "test_cases" in fn_lower:
                left = row[0:min(len(row), 5)]
                while len(left) < 5:
                    left.append("N/A")
                    
                frameworks = ["playwright", "selenium", "cypress", "rest assured", "postman", "sql validation", "manual"]
                fw = "N/A"
                auto_cand = "Yes"
                sev = "Medium"
                prio = "P2"
                ...
                for item in tail:
                    ...
                    if item_low in frameworks:
                        fw = item_clean
                    elif item_clean in ["Yes", "No"]:
                        auto_cand = item_clean
                    elif item_clean in ["Critical", "High", "Medium", "Low"]:
                        sev = item_clean
                    elif re.match(r'^P[0-3]$', item_clean, re.I):
                        prio = item_clean.upper()
                    elif item_clean in ["Not Executed", "Passed", "Failed", "Blocked"]:
                        status = item_clean
                    elif item_clean in ["QA", "Automation"]:
                        exec_by = item_clean
                    else:
                ...
                if len(non_meta) >= 4:
                    ...
                    pre_cond, steps, exp_res, act_res = "N/A", "N/A", "N/A", "N/A"
                    
                repaired_row = left + [pre_cond, steps, exp_res, act_res, status, exec_by, prio, sev, auto_cand, fw]
    ...
    def generate_test_documents(self):
        ...
        # Check if we can resume or create a new version directory
        ...
        if latest_dir:
            existing_docs = [
                ...
                if (f.endswith(".md") or f.endswith(".csv") or f.endswith(".txt"))
                and os.path.getsize(os.path.join(latest_dir, f)) > 0
            ]
            ...
            for name in ["test_cases.csv", "test_plan.md"]:
                ...
                if not any(os.path.exists(p) and os.path.getsize(p) > 0 for p in possible_paths):
                    p1_files_exist = False
                    break

            p2_files_exist = True
            for p_file in phase2_prompts:
                exp_name = self._get_expected_filename_from_prompt(p_file)
                p = os.path.join(latest_dir, exp_name)
                if not (os.path.exists(p) and os.path.getsize(p) > 0):
                    p2_files_exist = False
                    break

            if not existing_docs:
                out_dir = latest_dir
                logger.info(f"Latest version directory {out_dir} contains no data. Reusing {out_dir}.")
            elif p1_files_exist and p2_files_exist:
                logger.info(f"Latest version directory {latest_dir} already contains all documents ({len(existing_docs)} files).")
                logger.info("Creating a new version folder for fresh re-generation.")
                out_dir = None
            else:
                resume_mode = True
                out_dir = latest_dir
                logger.info(f"[Checkpoint] Execution stopped in middle of previous run in {out_dir}.")
                logger.info(f"[Checkpoint] Resuming generation workflow from last completed checkpoint ({len(existing_docs)} existing files detected).")
        ...
        if resume_mode and p1_files_exist:
            ...
            for i, batch in enumerate(batches):
                ...
                def _process_phase1_prompt(st):
                    ...
                    try:
                        ...
                        logger.error(f"  ✗ Failed Phase 1 artifact {st['base_name']} (Batch {batch_num}): {e}")

                max_p1_workers = min(len(phase1_state), 4) or 2
                with ThreadPoolExecutor(max_workers=max_p1_workers) as executor:
                    futures = [executor.submit(_process_phase1_prompt, st) for st in phase1_state]
        ...
        def _generate_phase2_doc(idx, p_file):
            base_name = os.path.basename(p_file)
        ...
        max_workers = min(len(phase2_prompts), 4) or 3
        logger.info(f"Running Phase 2 multithreaded with up to {max_workers} concurrent threads...")
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = [executor.submit(_generate_phase2_doc, idx, p_file) for idx, p_file in enumerate(phase2_prompts, 1)]
            for future in as_completed(futures):
                res = future.result()
                if res and res not in generated_files:
        ...
        logger.info("------------------------------------------------------------")
        logger.info("Generation Loop Engineering & Review Status:")
        recomm_file = os.path.join(out_dir, "automation_recommendations.csv")
        if os.path.exists(recomm_file):
            logger.info(f"[Generation Loop Engineering] Recommendations generated in {recomm_file} require human engineering review and approval.")
        logger.info(f"[Action Required] Review generated test artifacts in {out_dir}.")
        logger.info(f"[Next Step] Trigger webhook call /webhook/human-review (or action='review') for human review acceptance before final test execution.")
        logger.info("=" * 60)
